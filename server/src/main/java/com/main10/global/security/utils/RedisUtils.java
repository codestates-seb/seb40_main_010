...
public class RedisUtils {
    ...
    public Object getData(String key, String provider) {
        String refreshKey = provider + "RT :" + key;
        return Optional.ofNullable(redisTemplate.opsForValue().get(refreshKey))
                .orElseThrow(() -> {
                    AuthException e = new AuthException(ExceptionCode.TOKEN_NOT_FOUND);
                    sendErrorToDiscord.sendAuthExceptionToDiscord(e);
                    throw e;
                });
    }
    ...
    public String isBlackList(String key) {
        ...
        Optional<String> findId = Optional.ofNullable(stringValueOperations.get(key));
        return findId.orElse(null);
    }
    ...
    public Long getId(String refreshToken) {
        ...
        ValueOperations<String, String> stringValueOperations = redisBlackListTemplate.opsForValue();
        String findId = Optional.ofNullable(stringValueOperations.get(refreshToken))
                .orElseThrow(() -> {
                    AuthException e = new AuthException(ExceptionCode.MISMATCH_ACCESS_TOKEN);
                    sendErrorToDiscord.sendAuthExceptionToDiscord(e);
                    throw e;
                });
        return Long.valueOf(String.valueOf(findId));
    }
}
